# Upgrading an existing alt-tv-rewind clone

This repo (a project cloned from **alt-tv-rewind**) predates a round of bug fixes and
...
> say `alt-tv-rewind`/`alt_tv_rewind`/`Alt Tv Rewind` — search-and-replace those to your
> project's name after copying (the same three tokens `bun run init` rewrites).

**Upstream** = a fresh clone of the latest alt-tv-rewind. Keep one handy to copy new
files from (`git clone <alt-tv-rewind-url> /tmp/alt-tv-rewind-upstream`).
